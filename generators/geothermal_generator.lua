...
local function furnace_node_timer(pos, elapsed)
		...
		if fuel_time < fuel_totaltime then
			...
			if true then
				...
				if fuel.time == 0 then
                    ...
                    -- Move the now empty fluid cell to the dst inventory
                    if inv:room_for_item("dst", {name="itest2:fluid_cell_empty", count=1, wear=0, metadata=""}) then
                        inv:add_item("dst", {name="itest2:fluid_cell_empty", count=1, wear=0, metadata=""})
                    end
					update = true
	...
	if fuel_totaltime ~= 0 then
		...
		swap_node(pos, "itest2:geothermal_generator_active")
		...
		swap_node(pos, "itest2:geothermal_generator")
...
minetest.register_node("itest2:geothermal_generator", {
	description = "Geothermal Generator",
    tiles = {
        "itest_generator_side.png", "itest_generator_side.png", "itest_generator_side.png",
        "itest_generator_side.png", "itest_generator_side.png", "itest2_geothermal_generator_front.png"},
	...
	on_blast = function(pos)
		...
		drops[#drops+1] = "itest2:geothermal_generator"
...
minetest.register_node("itest2:geothermal_generator_active", {
	description = "Geothermal Generator",
    tiles = {
        "itest_generator_side.png", "itest_generator_side.png", "itest_generator_side.png",
        "itest_generator_side.png", "itest_generator_side.png", "itest2_geothermal_generator_front.png"},
	...
	paramtype2 = "facedir",
	light_source = 8,
	drop = "itest2:geothermal_generator",
	legacy_facedir_simple = true,
	is_ground_content = false,
	sounds = default.node_sound_stone_defaults(),
	on_timer = furnace_node_timer,
...
tech_api.energy.register_device("itest2:geothermal_generator_active", "default", {
  class = { 'default' },
  type = 'provider',
  max_rate = 100,
  linkable_faces = {'back', 'top', 'left', 'right', 'bottom'},
  callback = function(pos, dtime, request)
    local produce = math.min(request, 100)
...
tech_api.energy.register_device("itest2:geothermal_generator", "default", {
  class = { 'default' },
  type = 'provider',
  max_rate = 100,
  linkable_faces = {'back', 'top', 'left', 'right', 'bottom'},
  callback = function(pos, dtime, request)
      if minetest.get_node(pos).name == "itest2:geothermal_generator_active" then
          local produce = math.min(request, 100)
          minetest.get_meta(pos):set_string("infotext", "request=" .. request .. "EU/t - producing=" .. produce .. "EU/t")
          return produce, 8
      else
